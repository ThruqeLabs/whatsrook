...
var errAppDataUnavailable = errors.New("wacaller: call app-data stream is unavailable")
...
func decodeReactionInfo(message []byte) (appDataReaction, error) {
	...
	for len(message) > 0 {
		...
		case num == 2 && typ == protowire.BytesType:
			...
			if n >= 0 {
				if !utf8.Valid(raw) {
					return appDataReaction{}, errors.New("wacaller: reaction is not valid UTF-8")
				}
	...
	if reaction.transactionID == 0 {
		return appDataReaction{}, errors.New("wacaller: reaction has no transaction ID")
	}
	return reaction, nil
}
...
func (s *appDataSender) sendReaction(emoji string) error {
	if !utf8.ValidString(emoji) {
		return errors.New("wacaller: reaction is not valid UTF-8")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
