...
func newEngine(c *Client) *engine {
	...
	if c != nil {
		...
		e.requestCallNode = func(ctx context.Context, node waBinary.Node, requestID string) (*waBinary.Node, error) {
			waiter := c.waitResponse(requestID)
			defer c.cancelResponse(requestID, waiter)
			...
			case response := <-waiter:
				if response == nil {
					return nil, errors.New("whatsmeow: call request returned no response")
				}
				return response, nil
			case <-ctx.Done():
				return nil, ctx.Err()
...
func (e *engine) requireRawCallAdapter() error {
	...
	if e.rawCallHookErr != nil {
		return fmt.Errorf("wacaller: raw call adapter is unavailable: %w", e.rawCallHookErr)
	}
	return nil
}
...
func (e *engine) install() {
	if err := e.installCallAckHook(); err != nil {
		...
		e.mu.Unlock()
		e.log.Error().Err(err).Msg("raw call adapter is unavailable")
	}
	e.c.AddEventHandler(func(evt any) {
		switch ev := evt.(type) {
...
func (e *engine) sendReaction(callID, emoji string) error {
	...
	if m == nil || m.call == nil || m.call.State() == CallPhaseEnded {
		e.mu.Unlock()
		return errors.New("wacaller: call is not active")
	}
	sender := m.appDataTx
	e.mu.Unlock()
	if sender == nil {
...
func (e *engine) transitionVideo(callID string, transition int) error {
	...
	case signaling.VideoStateUpgradeAccept:
		if !m.peerVideoUpgrade {
			e.mu.Unlock()
			return errors.New("wacaller: no pending peer video upgrade")
		}
	...
	default:
		e.mu.Unlock()
		return fmt.Errorf("wacaller: unsupported local video transition %d", transition)
	}
	e.mu.Unlock()
	if sender != nil {
		switch transition {
...
func (e *engine) setVideoOrientation(callID string, orientation int) error {
	if orientation < 0 || orientation > 3 {
		return fmt.Errorf("wacaller: video orientation %d is outside 0..3", orientation)
	}
	e.mu.Lock()
	m := e.calls[callID]
	if m == nil || m.call == nil || m.call.State() == CallPhaseEnded {
...
func (e *engine) placeCall(ctx context.Context, target string, opts CallOptions) (*Call, error) {
	...
	if self.IsEmpty() {
		return nil, errors.New("wacaller: no own LID on this session")
	}
	...
	e.log.Info().Str("call_id", callID).Bool("video", opts.Video).Msg("offer sent; media starts when the relay endpoint arrives")
	e.diag.Emit("meta", map[string]any{"event": "offer_sent", "call_id": callID, "peer_lid": peerLID.String(), "direction": "out", "video": opts.Video})
	return call, nil
}
...
func (e *engine) onOffer(ev *events.CallOffer) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/33854919e64bdd4b053054ac9764d8fc63027b57/datasheets/voip-group-invite-accept.md#L28-L40
	groupSnapshot, isGroup, groupErr := signaling.ParseGroupInviteSnapshot(ev.Data)
	if groupErr != nil {
		e.log.Warn().Err(groupErr).Str("call_id", ev.CallID).Msg("parse inbound group offer failed")
		return
	}
	if isGroup {
	...
	if oag.OptionalString("is_call_ended") == "1" || oag.OptionalString("terminate_reason") != "" {
		e.log.Warn().Str("call_id", ev.CallID).Msg("ignoring already-ended offer; not a live call")
		return
	}
	...
	if raw := oag.OptionalString("e"); raw != "" {
		if elapsed, err := strconv.Atoi(raw); err == nil && elapsed > 90 {
			e.log.Warn().Str("call_id", ev.CallID).Int("elapsed_s", elapsed).Msg("ignoring stale offer replayed from the offline queue")
			return
		}
	}

	callKey, err := decryptInboundCallKey(context.Background(), e.c, ev)
	if err != nil {
	...
	e.diag.Emit("meta", map[string]any{
	...
	m.call = call
	m.callKey = callKey
	m.selfLID = e.c.Store.GetLID().String()
	...
	m.inviteSelfDevice = groupCallDevice{
		JID: e.c.Store.GetLID(), CapabilityVersion: 1,
		Capability: append([]byte(nil), signaling.CapabilityOffer...),
	}
	...
	if err := e.sendPreaccept(ev.CallID, ev.From, ev.CallCreator, isVideo); err != nil {
		e.log.Warn().Err(err).Str("call_id", ev.CallID).Msg("preaccept failed")
	}
...
func (e *engine) onGroupOffer(ev *events.CallOffer, update groupCallUpdate) {
	...
	if err != nil {
		e.log.Warn().Err(err).Str("call_id", ev.CallID).Msg("build group preaccept failed")
		return
	}
...
func (e *engine) answer(c *Call) error {
	m := e.lookup(c.id)
	if m == nil {
		return fmt.Errorf("wacaller: unknown call %s", c.id)
	}
	if m.group {
		// Source of truth: https://github.com/purpshell/wacaller/blob/676ebee3eca513b5348fab36cae5c560cc791238/datasheets/voip-group-invite-accept.md#L26-L45
		...
		if err = e.transmitCallNode(context.Background(), accept); err != nil {
			return fmt.Errorf("wacaller: send group accept: %w", err)
		}
...
func (e *engine) sendAccept(callID string, to, creator types.JID) {
	...
	accept.Attrs["id"] = e.c.GenerateMessageID()
	if err := e.c.sendNode(context.Background(), accept); err != nil {
		e.log.Error().Err(err).Str("call_id", callID).Msg("send accept failed")
		return
	}
	e.log.Info().Str("call_id", callID).Bool("video", isVideo).Msg("accepted (after mute_v2)")
	...
	if pending != nil {
		if err := e.setVideoEnabled(callID, *pending); err != nil {
			e.log.Warn().Err(err).Str("call_id", callID).Msg("deferred video enable failed")
		}
	}
	if pendingOrientation != nil {
		if err := e.setVideoOrientation(callID, *pendingOrientation); err != nil {
			e.log.Warn().Err(err).Str("call_id", callID).Msg("deferred video orientation failed")
		}
	}
}
...
func (e *engine) onRelayLatency(ev *events.CallRelayLatency) {
	...
	for i := range rl.GetChildren() {
		...
		if len(offered) > 0 && !offered[name] {
			e.log.Debug().Str("call_id", ev.CallID).Str("relay_name", name).Msg("skipping latency response for relay outside the offer")
			continue
		}
	...
	for _, p := range probes {
		...
		resp.Attrs["id"] = e.c.GenerateMessageID()
		if err := e.c.sendNode(context.Background(), resp); err != nil {
			e.log.Error().Err(err).Str("call_id", ev.CallID).Msg("send relaylatency failed")
			return
		}
	}
}
...
func (e *engine) onAccept(ev *events.CallAccept) {
	...
	if m.group {
		...
		e.log.Info().Str("call_id", ev.CallID).Str("from", ev.From.String()).Msg("participant accepted group call")
...
func inviteDeviceCapability(device types.JID, node *waBinary.Node) (groupCallDevice, bool) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/1ebd064663ac336ff3d1fc65d9baa974148fe73e/datasheets/voip-group-participant-invite.md#L36-L72
	if device.IsEmpty() {
...
func (e *engine) applyVoipSettingsCodec(m *engineCall, node *waBinary.Node, callID string) {
	...
	vs, err := signaling.ParseVoipSettings(content, e.log)
	if err != nil {
		e.log.Debug().Err(err).Str("call_id", callID).Msg("voip_settings parse failed; keeping mlow")
		return
	}
	...
	if errCode := ack.AttrGetter().String("error"); errCode != "" {
		...
		if callID == "" {
			...
			for id, m := range e.calls {
				if m != nil && m.direction == CallDirectionOutgoing && m.relay == nil && !m.started {
					callID = id
					break
				}
			}
		...
		e.log.Warn().Str("call_id", callID).Str("error_code", errCode).Msg("call rejected by server")
		e.finishCall(callID, "server:"+errCode)
		return
	}
	groupUpdate, isGroup, groupErr := signaling.ParseInitialGroupCallAck(ack)
	if groupErr != nil {
		e.log.Warn().Err(groupErr).Msg("parse initial group call ack failed")
		return
	}
	if isGroup {
		// Source of truth: https://github.com/purpshell/wacaller/blob/7cb6045001dafd2514f53e85cd8c3e419c13adbe/datasheets/voip-initial-group-call.md#L27-L33
...
func (e *engine) onCallRaw(callNode *waBinary.Node) bool {
	...
	case "group_update", "enc_rekey", "waiting_room_update", "user_action", "screen_share":
		// Source of truth: https://github.com/purpshell/wacaller/blob/36d54857c74e45ccb08f6444a32d2afa13f20be9/datasheets/group-video-reactions.md#L31-L56
...
func (e *engine) ackVideoStanza(callNode *waBinary.Node) {
	...
	if !ok {
		e.log.Warn().Msg("video ack: missing id/from, not acking")
		return
	}
	if err := e.c.sendNode(context.Background(), ack); err != nil {
		e.log.Warn().Err(err).Str("id", ack.AttrGetter().String("id")).Msg("send video ack failed")
		return
	}
	e.log.Debug().Str("id", ack.AttrGetter().String("id")).Msg("sent type=video ack")
}
...
func (e *engine) onVideoStanza(v *waBinary.Node) {
	...
	e.log.Debug().Str("call_id", callID).Int("state", state).Int("orientation", orientation).Msg("inbound video state")
	e.mu.Lock()
	m := e.calls[callID]
	if m == nil {
		e.mu.Unlock()
	...
	if announceEnabled {
		...
		if err := e.transmitCallNode(context.Background(), node); err != nil {
			...
			announceEnabled = false
			if e.c != nil {
				e.log.Warn().Err(err).Str("call_id", callID).Msg("video enabled announcement failed")
			}
		} else {
			enableSender = true
			requestKeyframe = true
...
func (e *engine) onTerminate(callID, reason string) {
	e.log.Info().Str("call_id", callID).Str("reason", reason).Msg("call terminated")
...
func (e *engine) finishCall(callID, reason string) {
	...
	if sink != nil {
		if err := sink.Close(); err != nil {
			e.log.Warn().Err(err).Str("call_id", callID).Msg("close call audio sink failed")
		}
	}
...
// installCallAckHook wires the call ack handling.
func (e *engine) installCallAckHook() error {
	if e == nil || e.c == nil {
		return errors.New("wacaller: raw call adapter has no whatsmeow client")
	}
	existing := e.c.RawNodeHandler
	e.c.RawNodeHandler = func(ctx context.Context, node *waBinary.Node) (*waBinary.Node, bool) {
		if existing != nil {
			var drop bool
			node, drop = existing(ctx, node)
			if drop || node == nil {
				return node, true
			}
		}
...
func resolvePeerLID(ctx context.Context, cli *Client, target string) (types.JID, error) {
...
func encryptCallKeyForDevice(ctx context.Context, cli *Client, dev types.JID, callKey []byte) ([]byte, string, bool, error) {
...
func decryptInboundCallKey(ctx context.Context, cli *Client, ev *events.CallOffer) ([]byte, error) {
	if ev.Data == nil {
