...
func (e *engine) placeGroupCall(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/7cb6045001dafd2514f53e85cd8c3e419c13adbe/datasheets/voip-initial-group-call.md#L63-L101
	if err := e.requireRawCallAdapter(); err != nil {
		return nil, err
	}
	self := e.c.Store.GetLID()
	if self.IsEmpty() {
	...
	if err = e.transmitCallNode(ctx, node); err != nil {
		...
		return nil, fmt.Errorf("wacaller: send group offer: %w", err)
	}
	e.log.Info().
		Str("call_id", callID).
...
func (e *engine) inviteParticipant(ctx context.Context, callID, target string) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/25eda415afb0f926112ca375c5892b95b4bd6f60/datasheets/voip-group-invite-offer.md#L81-L106
	if err := e.requireRawCallAdapter(); err != nil {
		return err
	}
	targetLID, err := resolvePeerLID(ctx, e.c, target)
	if err != nil {
		return err
	}
	...
	if targetLID == self.ToNonAD() {
		return errors.New("wacaller: cannot add this client to its own call")
	}
	...
	for _, participant := range participants {
		if participant.JID.ToNonAD() == targetLID {
			return errors.New("wacaller: participant is already in the call roster")
		}
	}
	...
	node.Attrs["id"] = e.nextCallNodeID()
	if err = e.transmitCallNode(ctx, node); err != nil {
		return fmt.Errorf("wacaller: add participant: %w", err)
	}
	return nil
}
...
func (e *engine) ringParticipant(ctx context.Context, callID, target string) error {
	...
	for _, participant := range participants {
		...
		if participant.State == "connected" {
			return errors.New("wacaller: participant is already connected")
		}
		found = true
	}
	if !found {
		return errors.New("wacaller: participant is not in the call roster")
	}
	if len(connected) == 0 {
		return errors.New("wacaller: group call has no connected participant roster")
	}
	targetDevices, err := e.discoverTargetDevices(ctx, targetLID)
	if err != nil {
		return err
	...
	node.Attrs["id"] = e.nextCallNodeID()
	if err = e.transmitCallNode(ctx, node); err != nil {
		return fmt.Errorf("wacaller: ring participant: %w", err)
	}
	return nil
}
...
func (e *engine) groupInviteRoster(
	...
	if m == nil || m.call == nil || m.call.State() == CallPhaseEnded {
		return types.EmptyJID, nil, false, errors.New("wacaller: call is not active")
	}
	if m.call.State() != CallPhaseActive {
		return types.EmptyJID, nil, false, errors.New("wacaller: call is not connected")
	}
	if m.creator.IsEmpty() {
		return types.EmptyJID, nil, false, errors.New("wacaller: call creator is unavailable")
	}
	...
	if m.inviteSelfDevice.JID.IsEmpty() || len(m.inviteSelfDevice.Capability) == 0 {
		return types.EmptyJID, nil, false, errors.New("wacaller: local active device capability is unavailable")
	}
	if m.invitePeerDevice.JID.IsEmpty() || len(m.invitePeerDevice.Capability) == 0 {
		return types.EmptyJID, nil, false, errors.New("wacaller: peer active device capability is unavailable")
	}
	self, err := types.ParseJID(m.selfLID)
	if err != nil || self.IsEmpty() {
		return types.EmptyJID, nil, false, errors.New("wacaller: local call identity is unavailable")
	}
	peer, err := types.ParseJID(m.peerLID)
	if err != nil || peer.IsEmpty() {
		return types.EmptyJID, nil, false, errors.New("wacaller: peer call identity is unavailable")
	}
...
func (e *engine) discoverTargetDevices(ctx context.Context, target types.JID) ([]types.JID, error) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/25eda415afb0f926112ca375c5892b95b4bd6f60/datasheets/voip-group-invite-offer.md#L81-L106
	devices, err := e.c.GetUserDevices(ctx, []types.JID{target})
	if err != nil {
		return nil, fmt.Errorf("wacaller: call invite device discovery: %w", err)
	}
	if len(devices) == 0 {
		return nil, fmt.Errorf("wacaller: call invite target %s has no devices", target)
	}
	return devices, nil
}
...
func (e *engine) resolveGroupTargets(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/ceaa2156015e8f24e09328fb7a9c89203295efff/datasheets/api-initial-group-call.md#L82-L99
	selected := make([]types.JID, 0, len(targets))
	seen := make(map[types.JID]struct{}, len(targets))
	for i, target := range targets {
		participant, err := resolvePeerLID(ctx, e.c, target)
		if err != nil {
			return nil, fmt.Errorf("wacaller: resolve group target %d: %w", i, err)
		}
		participant = participant.ToNonAD()
		if participant == self.ToNonAD() {
			return nil, fmt.Errorf("wacaller: group target %d is this client", i)
		}
	...
	if len(selected) < 2 {
		return nil, errors.New("wacaller: group call requires at least two distinct remote targets")
	}
	if len(selected) > 31 {
		return nil, errors.New("wacaller: group call supports at most 31 remote targets")
	}
	return selected, nil
}
...
func (e *engine) discoverGroupParticipants(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/7cb6045001dafd2514f53e85cd8c3e419c13adbe/datasheets/voip-initial-group-call.md#L63-L101
	devices, err := e.c.GetUserDevices(ctx, users)
	if err != nil {
		return nil, fmt.Errorf("wacaller: group device discovery: %w", err)
	}
	...
	for _, user := range users {
		...
		if len(userDevices) == 0 {
			return nil, fmt.Errorf("wacaller: participant %s has no reachable devices", user)
		}
...
func parseOptionalGroupJID(raw string) (types.JID, error) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/ceaa2156015e8f24e09328fb7a9c89203295efff/datasheets/api-initial-group-call.md#L82-L96
...
func selectedGroupCallState(targets []types.JID) *GroupCallState {
	// Source of truth: https://github.com/purpshell/wacaller/blob/ceaa2156015e8f24e09328fb7a9c89203295efff/datasheets/api-initial-group-call.md#L97-L104
...
func (e *engine) joinPublicCallLink(
	...
	if joined.CallID == "" {
		return nil, errors.New("wacaller: call-link join returned no call ID")
	}
...
func (e *engine) setHandRaised(callID string, raised bool) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/36d54857c74e45ccb08f6444a32d2afa13f20be9/datasheets/group-video-reactions.md#L31-L43
	if err := e.requireRawCallAdapter(); err != nil {
		return err
	}
	creator, err := e.callControlCreator(callID)
	...
	participant := e.c.Store.GetLID().ToNonAD()
	e.mu.Unlock()
	if call != nil {
...
func (e *engine) setScreenShare(callID string, active bool, screenShareID *uint32) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/36d54857c74e45ccb08f6444a32d2afa13f20be9/datasheets/group-video-reactions.md#L44-L56
	if err := e.requireRawCallAdapter(); err != nil {
		return err
	}
	creator, err := e.callControlCreator(callID)
...
func (e *engine) callControlCreator(callID string) (types.JID, error) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/699185f41519da3177c17ea6a10f9d4aa48b6941/datasheets/voip-group-call-state.md#L62-L68
	e.mu.Lock()
	defer e.mu.Unlock()
	m := e.calls[callID]
	if m == nil || m.call == nil || m.call.State() == CallPhaseEnded || m.creator.IsEmpty() {
		return types.EmptyJID, errors.New("wacaller: group call is not active")
	}
		...
		for {
			...
			case <-ticker.C:
				...
				if err := e.transmitCallNode(ctx, node); err != nil && ctx.Err() == nil {
					e.log.Debug().Err(err).Str("call_id", callID).Msg("waiting-room heartbeat failed")
				}
			}
		}
	}()
