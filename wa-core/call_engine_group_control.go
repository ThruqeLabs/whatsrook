...
func (e *engine) onUnknownCallEvent(node *waBinary.Node) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/48c2391ce9f7dcc2b3f223f72f1b5f0c627ad943/datasheets/voip-group-update-ingest.md#L105-L148
	...
	case "group_update":
		...
		if parseErr != nil {
			e.log.Warn().Err(parseErr).Str("call_id", envelope.CallID).Msg("parse group update failed")
			return
		}
		...
		if e.applyGroupUpdate(update) && update.RekeyRequested {
			if fanoutErr := e.distributeRequestedGroupEpoch(context.Background(), update); fanoutErr != nil {
				e.log.Warn().Err(fanoutErr).Str("call_id", update.CallID).Msg("group rekey fanout failed")
			}
		}
	case "waiting_room_update":
		room, parseErr := signaling.ParseWaitingRoomUpdate(&envelope.Action)
		if parseErr != nil {
			e.log.Warn().Err(parseErr).Str("call_id", envelope.CallID).Msg("parse waiting room update failed")
			return
		}
	...
	case "user_action":
		...
		if parseErr != nil {
			e.log.Warn().Err(parseErr).Str("call_id", envelope.CallID).Msg("parse hand state failed")
			return
		}
	...
	case "screen_share":
		...
		if parseErr != nil {
			e.log.Warn().Err(parseErr).Str("call_id", envelope.CallID).Msg("parse screen share failed")
			return
		}
	...
	case "enc_rekey":
		...
		if err = e.ingestGroupEpoch(context.Background(), envelope); err != nil {
			e.log.Warn().Err(err).Str("call_id", envelope.CallID).Msg("group rekey ingest failed")
			return
		}
		e.log.Debug().
			Str("call_id", envelope.CallID).
...
func (e *engine) applyGroupUpdate(update groupCallUpdate) bool {
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L83-L100
	if update.CallID == "" {
		return false
	}
	e.mu.Lock()
	...
	if receivers != nil {
		if err := receivers.ApplyGroupUpdate(stored); err != nil {
			e.log.Warn().Err(err).Str("call_id", update.CallID).Msg("apply group media roster failed")
			return false
		}
	}
...
func (e *engine) ingestGroupEpoch(ctx context.Context, envelope *signaling.CallControlEnvelope) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/747c6a1b8a0370358ef18bbaa5e029b960c2f836/datasheets/voip-group-enc-rekey-ingest.md#L39-L123
	if envelope == nil {
		return errors.New("wacaller: group rekey envelope is nil")
	}
	...
	if !ok {
		return errors.New("wacaller: group rekey has no encrypted payload")
	}
	plaintext, _, err := e.c.decryptDM(
		ctx,
		&enc,
		envelope.From,
	...
	if err != nil {
		return fmt.Errorf("wacaller: decrypt group rekey: %w", err)
	}
	defer clear(plaintext)
	var message waE2E.Message
	if err = proto.Unmarshal(plaintext, &message); err != nil {
		return fmt.Errorf("wacaller: decode group rekey: %w", err)
	}
	rawKey := message.GetCall().GetCallKey()
	if len(rawKey) != 32 {
		return fmt.Errorf("wacaller: group rekey key is %d bytes, want 32", len(rawKey))
	}
	defer clear(rawKey)
...
func (e *engine) installGroupEpoch(callID string, transactionID uint32, rawKey []byte) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/d9df3eb9d96ea5260ffcd4036b6669499a1c1bc2/datasheets/voip-group-key-epoch-fanout.md#L85-L143
	if callID == "" || transactionID == 0 || len(rawKey) != 32 {
		return errors.New("wacaller: invalid group key epoch")
	}
	e.mu.Lock()
	m := e.calls[callID]
	if m == nil || !m.group {
		e.mu.Unlock()
		return fmt.Errorf("wacaller: unknown group call %s", callID)
	}
	if m.hasGroupEpoch {
		switch {
		...
		case transactionID == m.groupEpochTxID:
			e.mu.Unlock()
			return fmt.Errorf("wacaller: conflicting group epoch for transaction %d", transactionID)
		}
	}
	receivers := m.groupReceivers
	e.mu.Unlock()
	...
	if m == nil || !m.group {
		e.mu.Unlock()
		return fmt.Errorf("wacaller: group call %s ended during rekey", callID)
	}
	if m.hasGroupEpoch {
		switch {
		case transactionID < m.groupEpochTxID:
...
func (e *engine) distributeRequestedGroupEpoch(ctx context.Context, update groupCallUpdate) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/d9df3eb9d96ea5260ffcd4036b6669499a1c1bc2/datasheets/voip-group-key-epoch-fanout.md#L99-L162
	self := e.c.Store.GetLID()
	recipients := groupRekeyRecipients(update, self)
	if len(recipients) == 0 {
		return errors.New("wacaller: group rekey has no remote connected devices")
	}
	rawKey := make([]byte, 32)
	...
	for i, recipient := range recipients {
		ciphertext, encType, _, err := encryptCallKeyForDevice(ctx, e.c, recipient, rawKey)
		if err != nil {
			clear(rawKey)
			return fmt.Errorf("wacaller: encrypt group epoch for %s: %w", recipient, err)
		}
	...
	for i, node := range nodes {
		if err := e.transmitCallNode(ctx, node); err != nil {
			fanoutErrors = append(
				fanoutErrors,
				fmt.Errorf("wacaller: send group epoch to %s: %w", recipients[i], err),
			)
		}
	}
	if err := e.installGroupEpoch(update.CallID, update.TransactionID, rawKey); err != nil {
		fanoutErrors = append(fanoutErrors, fmt.Errorf("wacaller: install local group epoch: %w", err))
	}
	clear(rawKey)
...
func groupRekeyRecipients(update groupCallUpdate, self types.JID) []types.JID {
	// Source of truth: https://github.com/purpshell/wacaller/blob/d9df3eb9d96ea5260ffcd4036b6669499a1c1bc2/datasheets/voip-group-key-epoch-fanout.md#L145-L162
...
func cloneGroupCallUpdate(update groupCallUpdate) groupCallUpdate {
	// Source of truth: https://github.com/purpshell/wacaller/blob/a9e4195fb846a730f30ce98c26a7d1c03993fdb2/datasheets/group-media-relay-refresh.md#L59-L69
	out := update
