...
func (e *engine) maybeStartMedia(callID string) {
	...
	if m.group {
		...
		if err != nil {
			e.mu.Unlock()
			e.log.Debug().Err(err).Str("call_id", callID).Msg("group media is waiting for an authoritative relay")
			return
		}
		rd = groupRelay
	}
	...
	go func() {
		...
		if err := e.runMedia(mctx, callID, call, callKey, selfLID, peerLID, rd, inbound); err != nil {
			e.log.Warn().Err(err).Str("call_id", callID).Msg("media ended")
		}
	}()
}
...
func (e *engine) connectAndAllocateAll(ctx context.Context, rd *relayData, streamSsrcs [9]uint32, inbound bool, multi bool) (*relayFanout, error) {
	...
	for _, ep := range targets {
		...
		if err != nil {
			...
			// no fallback is not.
			e.log.Warn().Err(err).Str("relay_name", ep.relayName).Msg("relay connect failed; continuing without it")
			continue
		}
...
func (e *engine) runMedia(ctx context.Context, callID string, call *Call, callKey []byte, selfLID, peerLID string, rd *relayData, inbound bool) error {
	...
	log.Info().
		...
		Msg("media session")
	e.diag.Emit("ssrc", map[string]any{
		...
		if groupMode.Load() {
			...
			for range groupRegistryInstallMaxAttempts {
				...
				if current == nil || !current.group || current.groupUpdate == nil ||
					...
					e.mu.Unlock()
					return fmt.Errorf("wacaller: group media prerequisites changed during startup")
				}
				...
				if current == nil || !current.group {
					e.mu.Unlock()
					return fmt.Errorf("wacaller: group call ended during media startup")
				}
			...
			if !installed {
				return fmt.Errorf(
					"wacaller: group media roster changed during %d startup attempts",
			...
			if current == nil {
				e.mu.Unlock()
				return fmt.Errorf("wacaller: direct call ended during media startup")
			}
			if current.groupReceivers == nil {
				current.groupReceivers = audioReceivers
				e.mu.Unlock()
	...
	e.diag.Emit("srtp", map[string]any{
	...
	go func() {
		...
		for {
			...
			e.diag.Emit("media_out", map[string]any{
	...
	appSender := newAppDataSender(txAppDataPipe, appDataSelfSsrc, func(packet []byte) (int, error) {
		if header, ok := rtp.ParseRtpHeader(packet); ok {
			e.diag.Emit("app_data", map[string]any{
	...
	if groupMode.Load() {
		...
		e.mu.Unlock()
		if update == nil || update.Relay == nil {
			return fmt.Errorf("wacaller: group media started without an authoritative relay snapshot")
		}
		var relayTx [12]byte
		if _, err := rand.Read(relayTx[:]); err != nil {
	...
	for {
		...
		if kind == mediaPayloadAppData {
			...
			if !ok {
				...
				e.diag.Emit("app_data", map[string]any{"event": "unprotect_failed", "ssrc": vh.Ssrc, "seq": vh.SequenceNumber})
				continue
			}
		...
		if kind == mediaPayloadVideo {
			...
			if !vunok {
				...
				e.diag.Emit("video", map[string]any{"event": "unprotect_failed", "ssrc": vh.Ssrc, "seq": vh.SequenceNumber})
				continue
			}
			vh = media.Header
			...
			if vh.VideoExtension != nil {
				...
				if orientation != videoState.orientation {
					...
					e.diag.Emit("video", map[string]any{
			...
			if videoWirePacket < videoWirePacketLimit {
				...
				e.diag.Emit("video_wire", map[string]any{
			...
			if complete {
				...
				videoWireFrame++
				e.diag.Emit("video", map[string]any{"event": "frame", "ssrc": vh.Ssrc, "bytes": len(frame)})
		...
		if kind != mediaPayloadAudio {
			...
			e.diag.Emit("rtp", map[string]any{
		...
		if !ok {
			...
			e.diag.Emit("srtp", map[string]any{"event": "unprotect_failed", "ssrc": vh.Ssrc, "bytes": n})
			continue
		}
		...
		e.diag.Emit("media_in", map[string]any{
...
func prepareWasmRelayStreamSSRCs(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/0911f20e97b858506a55ee6aa4f6a1ad73f19798/datasheets/group-video-reactions.md#L51-L65
	if random == nil {
		return [9]uint32{}, fmt.Errorf("wacaller: relay stream SSRC random source is nil")
	}
	...
	for i := 6; i < len(streamSSRCs); i++ {
		...
		for range 64 {
			...
			if _, err := io.ReadFull(random, encoded[:]); err != nil {
				return [9]uint32{}, fmt.Errorf("wacaller: generate auxiliary video SSRC: %w", err)
			}
		...
		if !selected {
			return [9]uint32{}, fmt.Errorf("wacaller: generate unique auxiliary video SSRC")
		}
	}
...
func sendMediaSrtcpReceptionReports(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/bab582d4e799292478ccba2f8a86f2164d4737c3/datasheets/group-media-rtcp-feedback.md#L148-L156
	if sender == nil || send == nil {
		return 0, fmt.Errorf("wacaller: SRTCP sender or send callback is nil")
	}
...
func handleGroupAppDataReaction(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/36d54857c74e45ccb08f6444a32d2afa13f20be9/datasheets/group-video-reactions.md#L10-L19
...
func (s *mediaSrtcpSender) groupSenderReport(stats rtp.RtcpSenderStats, nowMs uint64, report *rtp.RtcpReceptionReport) ([]byte, error) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/6e202a6d6ec5a9384bae6ccbe621966edeee6592/datasheets/group-media-rtcp-feedback.md#L53-L75
	s.mu.Lock()
	defer s.mu.Unlock()
