...
func newParticipantReceiveRegistry(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/ca4ba64503efeb86c337ee37cb00c4da540c632c/datasheets/group-media-receive.md#L24-L44
	...
	if err != nil {
		return nil, fmt.Errorf("wacaller: parse initial media peer: %w", err)
	}
...
func (r *participantReceiveRegistry) newReceiver(userJID, deviceJID types.JID, pid uint32, hasPID bool) (*participantAudioReceiver, error) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/ca4ba64503efeb86c337ee37cb00c4da540c632c/datasheets/group-media-receive.md#L37-L44
...
func (r *participantReceiveRegistry) ApplyGroupUpdateTransaction(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L100-L141
	if apply == nil {
		return fmt.Errorf("wacaller: group update apply callback is nil")
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if !committed {
		return fmt.Errorf("wacaller: group update apply completed without commit")
	}
	return nil
}

func (r *participantReceiveRegistry) ApplyGroupUpdate(update groupCallUpdate) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/ca4ba64503efeb86c337ee37cb00c4da540c632c/datasheets/group-media-receive.md#L83-L100
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L139-L141
...
func (r *participantReceiveRegistry) prepareGroupUpdateLocked(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L110-L120
	...
	for _, participant := range update.Participants {
		...
		for _, device := range participant.Devices {
			...
			// Source of truth: https://github.com/purpshell/wacaller/blob/e34f414e423e6e45863d19dcfdd7e658f5f1314a/datasheets/group-media-receive.md#L24-L42
			...
			if _, exists := prepared.byDeviceID[participantID]; exists {
				prepared.clear(true)
				return nil, fmt.Errorf("wacaller: duplicate group participant device %s", participantID)
			}
			...
			if _, exists := prepared.byPID[device.PID]; exists {
				prepared.clear(true)
				return nil, fmt.Errorf("wacaller: duplicate group participant PID %d", device.PID)
			}
			if _, exists := prepared.bySSRC[receiver.ssrc]; exists {
				prepared.clear(true)
				return nil, fmt.Errorf("wacaller: duplicate group participant SSRC %d", receiver.ssrc)
			}
			if _, exists := prepared.byVideoSSRC[receiver.videoSSRC]; exists {
				prepared.clear(true)
				return nil, fmt.Errorf("wacaller: duplicate group participant video SSRC %d", receiver.videoSSRC)
			}
			if _, exists := prepared.byAppDataSSRC[receiver.appDataSSRC]; exists {
				prepared.clear(true)
				return nil, fmt.Errorf("wacaller: duplicate group participant app-data SSRC %d", receiver.appDataSSRC)
			}
			for _, streamSSRC := range receiver.streamSSRCs {
				if _, exists := prepared.bySRTCPSSRC[streamSSRC]; exists {
					prepared.clear(true)
					return nil, fmt.Errorf("wacaller: duplicate group participant stream SSRC %d", streamSSRC)
				}
	...
	if hasNewestPending {
		...
		case newestPending.transactionID == selectedEpoch.transactionID &&
			...
			return nil, fmt.Errorf(
				"wacaller: conflicting group raw epoch for transaction %d",
...
func (r *participantReceiveRegistry) commitGroupUpdateLocked(prepared *preparedParticipantReceiveUpdate) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L122-L137
...
func (p *preparedParticipantReceiveUpdate) clear(rollback bool) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L117-L128
	if p == nil {
		return
	}
	if p.epoch != nil {
...
func clearParticipantReceiverKeys(receiver *participantAudioReceiver) {
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L117-L120
	if receiver == nil {
		return
	}
...
func (r *participantReceiveRegistry) HasCommittedGroupUpdate() bool {
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L117-L140
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *participantReceiveRegistry) attachSendPipeline(sendPipe *MediaPipeline) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/cbe1446dabb5842362b1a4362d4100ec15d8254f/datasheets/group-media-key-epoch.md#L78-L86
	if sendPipe == nil {
		return fmt.Errorf("wacaller: nil media send pipeline")
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.hasEpoch {
		...
		if err != nil {
			return fmt.Errorf("wacaller: derive late group send epoch: %w", err)
		}
...
func (r *participantReceiveRegistry) attachSRTCPSender(sender *mediaSrtcpSender) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/cbe1446dabb5842362b1a4362d4100ec15d8254f/datasheets/group-media-key-epoch.md#L78-L120
	if sender == nil {
		return fmt.Errorf("wacaller: nil SRTCP sender")
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.hasEpoch {
		...
		if err != nil {
			return fmt.Errorf("wacaller: derive late group SRTCP send epoch: %w", err)
		}
...
func (r *participantReceiveRegistry) ApplyGroupRawEpoch(transactionID uint32, rawKey []byte) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/cbe1446dabb5842362b1a4362d4100ec15d8254f/datasheets/group-media-key-epoch.md#L88-L120
	if len(rawKey) != 32 {
		return fmt.Errorf("wacaller: group raw epoch has %d bytes, want 32", len(rawKey))
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	if !r.hasGroupUpdate || transactionID > r.transactionID {
		if existing, ok := r.pendingEpochs[transactionID]; ok {
			...
			return fmt.Errorf("wacaller: conflicting group raw epoch for transaction %d", transactionID)
		}
...
func (r *participantReceiveRegistry) applyGroupRawEpochLocked(transactionID uint32, rawKey []byte) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/cbe1446dabb5842362b1a4362d4100ec15d8254f/datasheets/group-media-key-epoch.md#L101-L120
	if r.hasEpoch {
...
func (r *participantReceiveRegistry) installGroupRawEpochLocked(rawKey []byte) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/cbe1446dabb5842362b1a4362d4100ec15d8254f/datasheets/group-media-key-epoch.md#L104-L136
...
func (r *participantReceiveRegistry) prepareGroupRawEpochLocked(
	...
	// Source of truth: https://github.com/purpshell/wacaller/blob/65b1dbf33f365db7392e438c3e3bf3651decb6cf/datasheets/group-media-receive.md#L110-L126
	sendKeys, err := srtp.DeriveE2eKeysFromRaw(rawKey, r.selfID)
	if err != nil {
		return nil, fmt.Errorf("wacaller: derive group send epoch: %w", err)
	}
	sendRTCPKeys, err := srtp.DeriveE2eSRTCPKeysFromRaw(rawKey, r.selfID, r.log)
	if err != nil {
		return nil, fmt.Errorf("wacaller: derive group SRTCP send epoch: %w", err)
	}
	...
	for _, receiver := range receivers {
		...
		if deriveErr != nil {
			prepared.clear()
			return nil, fmt.Errorf("wacaller: derive group receive epoch: %w", deriveErr)
		}
		prepared.receiveKeys[receiver] = keys
		srtcpKeys, deriveErr := srtp.DeriveE2eSRTCPKeysFromRaw(rawKey, receiver.participantID, r.log)
		if deriveErr != nil {
			prepared.clear()
			return nil, fmt.Errorf("wacaller: derive group SRTCP receive epoch: %w", deriveErr)
		}
...
func (r *participantReceiveRegistry) RekeyFallback(peerLID string) error {
	// Source of truth: https://github.com/purpshell/wacaller/blob/ca4ba64503efeb86c337ee37cb00c4da540c632c/datasheets/group-media-receive.md#L89-L100
	peerJID, err := types.ParseJID(peerLID)
	if err != nil {
		return fmt.Errorf("wacaller: parse answering media peer: %w", err)
	}
	...
	if r.hasGroupUpdate {
		...
		return fmt.Errorf("wacaller: answering media peer is not active in group roster")
	}
	receiver, err := r.newReceiver(peerJID.ToNonAD(), peerJID, 0, false)
	if err != nil {
		return err
...
func (r *participantReceiveRegistry) ActiveAudioSSRCs() []uint32 {
	// Source of truth: https://github.com/purpshell/wacaller/blob/6e202a6d6ec5a9384bae6ccbe621966edeee6592/datasheets/group-media-rtcp-feedback.md#L133-L135
	r.mu.RLock()
	defer r.mu.RUnlock()
