...
func generateMsgSecretKey(
	...
	// Clamp each length individually before summing to prevent any possible
	// integer overflow in the capacity computation (CWE-190 / CodeQL #48).
	// Values are protocol strings; anything beyond 2048 bytes is treated as
	// untrusted / malformed, so we fall back to a zero pre-allocation.
	clamp := func(n, max int) int {
		if n > max {
			return 0
		}
		return n
	}
	l1 := clamp(len(origMsgID), 2048)
	l2 := clamp(len(origMsgSenderStr), 2048)
	l3 := clamp(len(modificationSenderStr), 2048)
	l4 := clamp(len(modificationType), 2048)
	// Each li is now at most 2048; their sum is at most 8192 — no overflow possible.
	totalCap := l1 + l2 + l3 + l4
	useCaseSecret := make([]byte, 0, totalCap)
