...
func (cli *Client) handlePictureNotification(_ context.Context, node *waBinary.Node) {
	ts := node.AttrGetter().UnixTime("t")
...
func (cli *Client) handleFBDeviceNotification(_ context.Context, node *waBinary.Node) {
	cli.userDevicesCacheLock.Lock()
	defer cli.userDevicesCacheLock.Unlock()
...
func (cli *Client) handleOwnDevicesNotification(_ context.Context, node *waBinary.Node, fromJID types.JID) {
	cli.userDevicesCacheLock.Lock()
	defer cli.userDevicesCacheLock.Unlock()
...
func (cli *Client) handleBlocklist(_ context.Context, node *waBinary.Node) {
	ag := node.AttrGetter()
...
func (cli *Client) handleNewsletterNotification(_ context.Context, node *waBinary.Node) {
	ag := node.AttrGetter()
...
func (cli *Client) handleMexNotification(_ context.Context, node *waBinary.Node) {
	for _, child := range node.GetChildren() {
		if child.Tag != "update" {
			continue
		}
...
func (cli *Client) handleStatusNotification(_ context.Context, node *waBinary.Node) {
	ag := node.AttrGetter()
