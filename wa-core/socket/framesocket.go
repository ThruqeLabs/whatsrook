...
func (fs *FrameSocket) SendFrame(data []byte) error {
	...
	dataLength := len(data)
	if dataLength < 0 || dataLength >= FrameMaxSize {
	...
	if headerLength < 0 || headerLength > 1024 {
		return fmt.Errorf("%w: header length too large (%d)", ErrFrameTooLarge, headerLength)
	}

	maxAllowedTotal := headerLength + FrameLengthSize + FrameMaxSize
	totalLength := headerLength + FrameLengthSize + dataLength
	if totalLength < 0 || totalLength > maxAllowedTotal {
		return fmt.Errorf("%w: total frame length overflow or exceeds limit (%d)", ErrFrameTooLarge, totalLength)
	}
